      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-10 md:gap-16 mb-20 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="space-y-5">
            ...
            {/* AJUSTE: Bajamos de text-6xl a text-4xl/5xl */}
        ...
        <div className="grid md:grid-cols-2 gap-6 md:gap-8 mb-24">
          {/* AJUSTE: Padding reducido de 12 a 8 */}
        ...
        {/* GRID DE VALORES: Altura reducida de 72 a 60 */}
        <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-24">
          {[
            ...
            <motion.div key={idx} variants={itemVariants} whileHover={{ y: -5 }} className="relative h-56 md:h-64 group cursor-pointer">
              <div className={`absolute inset-0 ${val.color} rounded-[35px] opacity-0 group-hover:opacity-100 transition-all shadow-xl`} />
              <div className="absolute inset-0 bg-white border border-slate-200 rounded-[35px] p-6 flex flex-col items-center justify-center text-center transition-all group-hover:bg-transparent group-hover:border-transparent">
                <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center mb-4 group-hover:bg-white/20 transition-all">
                  <div className={`${val.textColor} group-hover:text-white transition-colors`}>{val.icon}</div>
                </div>
                <span className="text-slate-900 font-black uppercase italic tracking-tighter text-base md:text-xl mb-2 group-hover:text-white leading-none">{val.title}</span>
                <p className="text-slate-800 font-bold text-[10px] md:text-xs opacity-0 group-hover:opacity-100 group-hover:text-white transition-all px-2">"{val.desc}"</p>
              </div>
            </motion.div>
          ))}
        </motion.div>
        ...
        <div className="mt-20">
          ...
          <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="columns-1 sm:columns-2 lg:columns-3 gap-4 space-y-4">
            {[
              { size: "h-56", title: "Terapia Grupal", color: "bg-fupagua-azul" },
              { size: "h-80", title: "Actividad al Aire Libre", color: "bg-fupagua-amarillo" },
              { size: "h-64", title: "Talleres para Padres", color: "bg-fupagua-verde" },
              { size: "h-80", title: "Aprendizaje Lúdico", color: "bg-fupagua-rojo" },
              { size: "h-56", title: "Integración Social", color: "bg-fupagua-azul" },
              { size: "h-72", title: "Celebraciones", color: "bg-fupagua-amarillo" }
            ].map((img, idx) => (
